...
You can create a new CMIP6 `EsgDict` object using the `esgdict()` function.
...
JSON file. It returns the full path of the saved JSON file.

```{r}
dict_file <- file.path(tempdir(), "CMIP6DICT.json")
dict$save(dict_file)
```

This file can be reloaded via `$load()` method to restore the last state of
current `EsgDict` object. It reads the JSON file and loads the CVs and Data
Request data.

```{r}
new_dict <- esgdict(project = "CMIP6")
new_dict$load(dict_file)
```
