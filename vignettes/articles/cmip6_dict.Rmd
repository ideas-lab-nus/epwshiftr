...
# Introduce the `EsgDict` class

{epwshiftr} provides an `EsgDict` class to help you fetch, parse and store
ESG project dictionaries. The current backend supports CMIP6 CVs and Data
Request.

## Create an `EsgDict` object

You can create a new `EsgDict` object using the `esgdict()` function. It
returns an empty dictionary object for the requested project. Currently,
`"CMIP6"` is the implemented project.
...
## Store and reuse the `EsgDict` object

`$save()` method stores all the core data of current `EsgDict` object into a
schema-validated JSON file named `CMIP6DICT.json` by default. It returns the
...
current `EsgDict` object. It reads the JSON file named `CMIP6DICT.json` by
default and loads the CVs and Data Request data.

```{r}
new_dict <- esgdict(project = "CMIP6")
