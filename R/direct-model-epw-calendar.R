...
hourmap__daily_offsets <- function(data) {
    offsets <- sort(unique(as.numeric(data[["cf_second_of_day"]])))
...
hourmap__circular_point_values <- function(data, target_days) {
    ...
    for (offset_index in seq_along(offsets)) {
        ...
        mapped[target_rows] <- stats::approx(
            x = c(source_phase - 1, source_phase, source_phase + 1),
            y = rep.int(source_value, 3L),
            xout = phase,
            method = "linear",
            rule = 2,
            ties = "ordered"
