...
isimip__target_day <- function(annual_phase, target_year_days) {
    ...
    pmin(
        as.integer(floor(annual_phase * target_year_days) + 1L),
        as.integer(target_year_days)
    )
}
...
isimip__circular_fill <- function(values) {
    ...
    n <- length(values)
    stats::approx(
        x = c(valid - n, valid, valid + n),
        y = rep(values[valid], 3L),
        xout = seq_len(n),
        method = "linear",
        rule = 2,
        ties = "ordered"
...
# Evaluate a method-local Normal, Gamma, or Weibull CDF. Gamma records are
# extended with a fixed location after using the shared zero-location fitter.
isimip__cdf <- function(fit, values) {
    ...
    shifted <- pmax(values - fit$parameters$location, 0)
    ...
    if (identical(fit$family, "weibull")) {
        return(stats::pweibull(
            shifted,
            shape = fit$parameters$shape,
            scale = fit$parameters$scale
        ))
    ...
    cli::cli_abort(
        "Unknown ISIMIP3BASD fitted distribution family {.val {fit$family}}."
    )
}

# Evaluate the inverse of one method-local fitted distribution.
isimip__quantile <- function(fit, probability) {
    if (identical(fit$family, "normal")) {
        return(stats::qnorm(
            probability,
            mean = fit$parameters$location,
...
isimip__ks_statistic <- function(values, fit) {
    ordered <- sort(values)
    probability <- isimip__cdf(fit, ordered)
    n <- length(ordered)
    max(c(
...
isimip__map_window <- function(
  ...
  resolved
    ...
    if (length(source_index) &&
        ...
        if (identical(resolved$mapping_model, "empirical")) {
            ...
            if (is.null(source_attempt$fit) ||
                ...
                raw_probability <- isimip__cdf(
                    source_attempt$fit,
                    source_map
                )
                probability <- pmin(
                    pmax(raw_probability, resolved$cdf_epsilon),
                    1 - resolved$cdf_epsilon
                )
                mapped[source_index] <- isimip__quantile(
                    target_attempt$fit,
                    probability
                )
    ...
    bounded <- pmin(
        pmax(mapped, resolved$bounds[[1L]]),
        resolved$bounds[[2L]]
    )
    list(
        value = bounded,
        diagnostics = list(
            ...
            mapping = mapping,
            clipped_values = as.integer(sum(bounded != mapped))
        )
    )
}
...
isimip__adjust_window <- function(
  ...
  variable
    ...
    bounded <- pmin(
        pmax(restored, resolved$bounds[[1L]]),
        resolved$bounds[[2L]]
    )
    if (!is.null(resolved$lower_threshold)) {
...
isimip__validate_result <- function(value, inputs, key) {
    if (!S7::S7_inherits(value, DailyAdjustedSeries)) {
        return(
            "ISIMIP3BASD must return a DailyAdjustedSeries object."
        )
    }
    if (!identical(value@output_role, "model_future")) {
        return(
            "ISIMIP3BASD output must retain the `model_future` role."
        )
    }
    TRUE
}
