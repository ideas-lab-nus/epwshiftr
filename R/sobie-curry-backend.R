...
sobie__backend_options <- function(options = NULL) {
    ...
    if (is.null(options)) {
        ...
        if (!is.list(options) || is.null(names(options)) ||
            ...
            cli::cli_abort("Sobie-Curry `options` must be a named list.")
        }
        unknown <- setdiff(names(options), names(defaults))
        if (length(unknown)) {
            cli::cli_abort(
                "Unknown Sobie-Curry option(s): {.val {unknown}}."
            )
        }
...
sobie__daily_statistics <- function(
    ...
    target_day <- pmin(
        target_year_days,
        floor(phase * target_year_days) + 1L
    )
...
sobie__register_components <- function() {
    components <- sobie__component_specs()
