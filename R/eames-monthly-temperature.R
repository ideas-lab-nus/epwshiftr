...
eames__monthly_temperature_options <- function(options = NULL) {
    ...
    if (is.null(options)) {
        ...
        if (!is.list(options) || is.null(names(options)) ||
            ...
            cli::cli_abort(
                "Eames monthly temperature `options` must be a named list."
            )
        }
        unknown <- setdiff(names(options), names(defaults))
        if (length(unknown)) {
            cli::cli_abort(
                "Unknown Eames monthly temperature option(s): {.val {unknown}}."
            )
        }
        options <- utils::modifyList(defaults, options)
    }

    # Reuse the established numerical and header-policy validation, injecting
    # the daily-only window solely for that validator and removing it again.
