...
sdm__future_blocks <- function(
  ...
  output_block_years
    ...
    if (length(years) > 1L && any(diff(years) != 1L)) {
        cli::cli_abort(
            "Scaled Distribution Mapping requires contiguous future model years."
        )
    }
...
sdm__validate_result <- function(value, inputs, key) {
    if (!S7::S7_inherits(value, DailyAdjustedSeries)) {
        return(
            "Scaled Distribution Mapping must return a DailyAdjustedSeries object."
        )
    }
    if (!identical(value@output_role, "model_future")) {
        return(
            "Scaled Distribution Mapping output must retain the `model_future` role."
        )
    }
    TRUE
}
...
sdm__register_component <- function() {
    component__register(sdm__component(), overwrite = TRUE)
}
