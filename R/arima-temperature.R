...
EPW_MORPH_ARIMA_TEMPERATURE_OPTIONS <- list(
    tolerance = 1e-8,
...
arima__temperature_options <- function(options = NULL) {
    ...
    if (is.null(options)) {
        ...
        if (!is.list(options) || is.null(names(options)) ||
            ...
            cli::cli_abort(
                "Arima temperature `options` must be a named list."
            )
        }
        unknown <- setdiff(names(options), names(defaults))
        if (length(unknown)) {
            cli::cli_abort(
                "Unknown Arima temperature option(s): {.val {unknown}}."
            )
        }
        options <- utils::modifyList(defaults, options)
    }

    # Reuse the shared validator with an internal placeholder window; Arima's
    # published percentile smoother remains fixed and is not exposed here.
...
arima__physics_apply <- function(data, inputs, context, options) {
    ...
    if (identical(policy, "paper_faithful")) {
        invalid <- weather[["relative_humidity"]] < 0 |
