...
build_cmip6_db <- function() {
    all_types <- c("activity", "experiment", "sub_experiment", "institution",
        "source", "table", "frequency", "grid_label", "realm", "source_type",
        "req_global_atts", "resolution")

    cvs <- lapply(all_types, fetch_cmip6_cv)
}

CMIP6DB <- R6::R6Class("CMIP6DB",
    public = list(

    ),

    private = list(
        url_base = "https://esgf-node.llnl.gov/search/cmip6/"
    )
)

# download.file(url_base, "esgf_llnl.html", mode = "wb", method = "libcurl")
# l <- readLines(url_base)

fetch_cmip6_cv <- function(type, save = FALSE) {
    assert_string(type)

    if (type %in% c("activity", "experiment", "sub_experiment", "institution", "source", "table")) {
        file_name <- sprintf("CMIP6_%s_id.json", type)
    } else if (type %in% c("frequency", "grid_label", "realm", "source_type")) {
        file_name <- sprintf("CMIP6_%s.json", type)
    } else if (type == "req_global_atts") {
        file_name <- "CMIP6_requried_global_attributes.json"
    } else if (type == "resolution") {
        file_name <- "CMIP6_nominal_resolution.json"
    } else {
        all_types <- c(
            "activity", "experiment", "sub_experiment", "institution", "source", "table",
            "frequency", "grid_label", "realm", "source_type",
            "req_global_atts", "resolution"
        )
        stop(sprintf("Invalid CMIP6 CV type found: '%s'. Should be one of [%s]",
            type, paste0("'", all_types, "'", collapse = ", ")
        ))
    }
    ...
    switch(type,
        activity = format_cmip6_cv_flat(q),
        experiment = format_cmip6_cv_experiment(q),
        frequency = format_cmip6_cv_flat(q),
        grid_label = format_cmip6_cv_flat(q),
        institution = format_cmip6_cv_flat(q),
        resolution = format_cmip6_cv_simple(q),
        realm = format_cmip6_cv_flat(q),
        req_global_atts = format_cmip6_cv_simple(q),
        source = format_cmip6_cv_source(q),
        source_type = format_cmip6_cv_flat(q),
        sub_experiment = format_cmip6_cv_flat(q),
        table = format_cmip6_cv_simple(q)
    )

}

format_cmip6_cv_simple <- function(json) {
    c(q$version_metadata, q[1L])
}

format_cmip6_cv_flat <- function(json) {
...
format_cmip6_cv_nest <- function(json) {
    val <- transpose_nested_list(json[[1L]], names(json)[1L])
    c(json$version_metadata, list(value = as.data.table(val)))
}

format_cmip6_cv_experiment <- function(json) {
    d <- format_cmip6_cv_nest(json)
...
format_cmip6_cv_source <- function(json) {
    d <- format_cmip6_cv_nest(json)
    setnames(d$value, 1L, "name")
    ...
    set(d$value, NULL, "model_component",
        lapply(d$value$model_component, function(comp) {
            as.data.table(lapply(transpose_nested_list(comp, "name"), unlist, FALSE, FALSE))
        })
    )
...
fetch_cmip6_mip_table <- function(tag = "latest", dir = ".") {
    assert_string(tag)
    assert_string(dir)
