...
# Locate the source checkout so the probe always tests the code beside it.
daily_probe__package_root <- function(script_path = daily_probe__script_path()) {
    ...
    if (!file.exists(description) ||
        !any(grepl("^Package:\\s*epwshiftr\\s*$",
            readLines(description, warn = FALSE)))) {
        stop("Cannot locate the epwshiftr source checkout.", call. = FALSE)
    }
    candidate
}
...
# Load the adjacent checkout so the probe and its receipt identify exact code.
