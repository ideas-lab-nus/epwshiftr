test_that("R6 EPW morphing backends can be looked up, registered, and selected", {
    expect_true("original_morphing" %in% epw_morph_backends())
    expect_true("original_morphing_absolute" %in% epw_morph_backends())
    expect_false("original_morphing_change_factor" %in% epw_morph_backends())

    original_morphing <- epw_morph_backend("original_morphing")
