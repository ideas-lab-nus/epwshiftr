...
test_that("daily target days use containing calendar-neutral intervals", {
    phase <- c(0, 0.5 / 365, 1 / 365, 364.5 / 365)

    expect_identical(
        daily__target_day(phase, 365L),
        c(1L, 1L, 2L, 365L)
    )
    expect_identical(
        daily__target_day(c(0, 359.5 / 360), 360L),
        c(1L, 360L)
    )
})

test_that("daily circular interpolation is continuous across year boundaries", {
    interpolated <- daily__circular_interpolate(
        coordinate = c(0.25, 0.5, 0.75),
        value = c(1, 2, 3),
        target = c(0, 0.25, 0.5, 0.75, 1)
    )

    expect_equal(interpolated, c(2, 1, 2, 3, 2))
    expect_error(
        daily__circular_interpolate(0.5, 1, 0.5, period = 0),
        "must be positive"
    )
})
