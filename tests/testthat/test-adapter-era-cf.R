...
test_that("identical reanalysis requests are reused across child stores", {
    cache <- tempfile("era-shared-cache-")
    withr::local_options(list(epwshiftr.dir_cache = cache))
    spec <- shift_era5(1999:2000)
    request <- list(
    ...
    retrieve <- function(dataset_id, request, target, reporter = NULL, ...) {
        ...
        list(
            path = target,
            job = list(request_id = "shared-job"),
            reused = FALSE
        )
    }
    ...
    first <- reanalysis__retrieve_cached(
        retrieve,
    ...
    second <- reanalysis__retrieve_cached(
        retrieve,
        "reanalysis-era5-single-levels",
        request,
        second_target,
    ...
    expect_identical(
        readBin(first$path, what = "raw", n = 10L),
        readBin(second$path, what = "raw", n = 10L)
    )
})
