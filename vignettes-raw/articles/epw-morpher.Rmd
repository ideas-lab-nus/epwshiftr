...
Methods and executable recipes are separate records:
...
| Recipe | A runnable composition of one method with explicit input roles, calendar handling, sequence, hourly reconstruction, physical policy, output calendar, diagnostics, and provenance | A paper's selected GCMs and periods, or a user evaluation design | `epw_morph_recipes()` |

Source selections, periods, scenarios, and evaluation metrics remain caller
owned. This keeps a published algorithm available without turning one paper's
experiment into the package default. Users can evaluate methods under a common
design by supplying the same eligible source data, periods, and EPW template to
their selected recipes. Method-specific inputs still remain explicit: for
example, Arima requires an observed reference while several other
daily-temperature methods do not.
...
algorithm, while the input, calendar, component, physical-policy, target-calendar,
diagnostic, and provenance fields make the execution boundary inspectable. A
method's intrinsic calendar statistic remains method-owned: Eames calculates
monthly mean/extrema changes and Arima calculates month-wise empirical
distributions. Inspecting the catalog does not run a recipe or serialize any
function:

```r
epw_morph_recipes()
epw_morph_recipe_spec("belcher_monthly")
epw_morph_recipe_spec("epwshiftr_monthly")
...
epw_morph_recipe_spec("sobie_curry_daily")
...
|:-------|:-------|:---------------|:--------|
| `belcher_monthly` | `paper_faithful` | Existing backend adapter | Belcher monthly workflow preserving the published field treatment |
| `epwshiftr_monthly` | `harmonized` | Existing backend adapter | Enhanced production workflow, including its optional historical-input fallback |
...
Eight daily bias-adjustment methods use one reusable temperature-to-EPW adapter:
Linear Scaling, Delta Change, Quantile Mapping, Quantile Delta Mapping, Scaled
Distribution Mapping, CDF-t, Equidistant CDF Matching, and ISIMIP3BASD. Their
recipes use the same daily `tas` role contract, 365-day calendar mapping, hourly
temperature reconstruction, specific-humidity closure, EPW output, and
diagnostics; only the signal component changes. The method catalog retains each
...
Those conditions are supplied by the user and are never applied implicitly.
...
```r
enhanced_monthly <- epw_morph_recipe(
    "epwshiftr_monthly",
    policy = "harmonized"
)
...
result. Source-specific substitutions reported by a paper are data-preparation
choices and are not applied automatically by the statistical method.
