...
columns for downstream analysis. `shift_summary()` groups output counts and
optional local weather statistics by scientific identity; it does not rank methods.
...
shift_history(store = store_dir(), type = "batch")
...
Dynamic batch views fit their terminal height and show failed and active children
before completed work. `shift_watch(batch, follow = FALSE, ui = shift_ui(detail = "detail"))`
prints every child, live activity, update time, diagnostic, and recovery action.
Errors appear before warnings, cancelled children have a separate count, and a
completed watch leaves its final receipt visible even when its return value is assigned.

`shift_history()` lists root runs, batch parents, and child runs with exact store
paths. Filter by `type = "run"` or `"batch"` and `status = c("failed", "partial")`.
This inspection is local and does not create a store; unreadable records remain
visible with status `unavailable` and an explanatory error.

`shift_summary(batch)` separates completed cases, recorded EPWs, and available
files for each method/model/scenario/period and member/grid. Add `weather = TRUE`
to read existing local EPWs and report temperature, humidity, wind speed, and
global horizontal radiation means. Each mean is weighted by valid hourly rows,
excludes EPW missing codes, and reports its own valid-hour count. Unreadable
files are counted with their errors. Compare output types, years, and valid-hour
coverage before comparing the means; these summaries do not establish scientific
equivalence between a representative year and a multi-year sequence.

For a standalone collect/extract/morph/EPW workflow, pass its saved `ShiftRun`
to `shift_summary()`. The summary recovers the method from the result's morphing
stage and groups the output manifest by scientific identity. Case counts cover
distinct cases with saved outputs, so multiple weather-year files for one case
do not inflate the case count.
...
ERA5 preparation appears as a `Calibration` operation with variable counts and
CDS request status. Morphing counts cases, while writing/export counts physical
EPW files. A started unit does not count as completed until its outcome arrives.
Batch discovery identifies the current method, node, and required variables.
...
Dynamic height uses `options(epwshiftr.ui_height = 24L)` when set; otherwise it
reads native POSIX terminal dimensions, then `LINES`, with a 24-row fallback. One row is reserved for
the prompt. Set the option for a smaller IDE pane; oversized generic frames
fall back to a compact status row, and static detail receipts remain complete.
