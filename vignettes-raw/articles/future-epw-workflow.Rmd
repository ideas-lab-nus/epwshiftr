...
The recommended user call selects Belcher as a reusable monthly transformation
and gives the run an explicit matching historical CMIP6 reference. A transform
...
frequencies, required input roles, and hourly reconstruction. Belcher is one
canonical published method. The enhanced package method is selected separately
...
`shift_morph()` call. Belcher requires matching historical and future model
inputs.
...
Matching historical CMIP6 data are required so Belcher can compute
