...
description: >
    Create future EPW files from a store-backed shift workflow.
---
...
This article runs a real store-native shift workflow from ESGF File records to
generated EPW files. It uses monthly `Amon` data so the live remote reads are
much smaller than an equivalent daily workflow.

Use a temporary store for this article run. For a real project, replace this
with a persistent project cache path.

```{r local-workspace}
workflow_root <- file.path(tempdir(), "epwshiftr-store-native-morphing")
...
<div class="shift-flow" role="img" aria-label="Store-native shift workflow from request to EPW outputs">
...
```{r dataset-collect-preview, eval=FALSE}
