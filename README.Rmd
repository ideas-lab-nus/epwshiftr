...
`shift_future_epw()`. It creates the CMIP6 request, extracts the site time
series, resolves the historical Belcher reference, writes EPW files inside the
store, and copies the final EPWs to `output`.
...
Use `shift_plan()` and `shift_explain()` when you want to inspect the workflow
before touching ESGF services.
