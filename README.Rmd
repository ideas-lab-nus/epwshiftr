...
f <- c(
    "tas_day_AWI-CM-1-1-MR_ssp585_r1i1p1f1_gn_20490101-20491231.nc",
...
* [Get started](#get-started)
    * [Recommended store-native workflow](#recommended-store-native-workflow)
...
For direct ESGF API work, start with `esg_query()` / `EsgQuery`. The older
`esgf_query()` remains available as a compatibility wrapper for legacy
data.table workflows, emits a gentle deprecation warning, and keeps its
legacy LLNL `host` semantics instead of `esg_query()`'s ORNL `index_node`
default.

### Recommended store-native workflow

The recommended high-level API is the `shift_*` workflow. Each step returns an
inspectable stage object and carries the store IDs needed by the next step.
...
Use `shift_diagnostics()`, `shift_coverage()`, and `shift_ids()` when you need
to inspect or tune a stage before continuing. The lower-level `EsgQuery`,
`EsgStore`, and `EpwMorpher` APIs remain available for advanced workflows.
