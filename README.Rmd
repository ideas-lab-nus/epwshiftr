...
`monthly_transform()`, `daily_transform()`, or `hourly_transform()`, then pass
run-specific climate and reference data separately. The function resolves
files, extracts the site time series, persists a resumable run, and copies only
the final EPWs to `dir`.
...
method can use the input EPW climatology; `monthly_transform("belcher")`
