...
epwshiftr helps you request climate projection data, download it into a local
store, extract site-level climate variables, morph a baseline EnergyPlus Weather
(EPW) file, and write shifted future EPW files. The recommended user-facing path
is the `shift_*` workflow.
...
request climate data, collect file records, download, extract, morph, and write
EPW outputs. Each stage returns an inspectable object and carries the internal
store IDs forward automatically.
...
performs live ESGF queries and downloads.
...
[store-native morphing workflow article](vignettes/articles/store-native-morphing-workflow.Rmd)
...
implementation. It is no longer the recommended README path.
