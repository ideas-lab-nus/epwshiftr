...
Foreground runs begin with a three-line intent summary. Dynamic
terminals then keep a four-row view of the stage, current unit,
case/output counts, and last event; redirected output uses scoped,
append-only log lines instead. Resolver attempts and user cases are
shown as compact tables at their workflow boundaries. `shift_ui()`
controls only presentation and never changes the workflow specification
or its `spec_hash`:
