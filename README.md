...
For direct ESGF API work, start with `esg_query()` / `EsgQuery`. The
older `esgf_query()` remains available as a compatibility wrapper for
...
# inspect CMIP6 data node status
(nodes <- data_node_status())
#>                          data_node status
#>                             <char> <char>
#>  1:                 aims3.llnl.gov     UP
